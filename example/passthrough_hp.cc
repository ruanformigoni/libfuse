...
static cxxopts::ParseResult parse_options(int argc, char **argv) {
    ...
    for (auto opt : flattened_mount_opts) {
        ...
        /* Filter out some obviously incorrect options. *time options are determined
         * by the underlying filesystem. remount is not supported by FUSE at the time.
         * sync is not supported by passthrough_hp (but could be) */
        if (opt == "fd" || opt == "relatime" || opt == "diratime" || opt == "lazytime" ||
            opt == "strictatime" || opt == "remount" || opt == "sync" || opt == "noatime" ||
            opt == "norelatime" || opt == "nodiratime" || opt == "nolazytime" ||
            opt == "nostrictatime" || opt == "nosync") {
